...
const Dashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState(
    location.state?.targetTab === "ADD_ROOM"
      ? user?.role === "LANDLORD"
        ? "ALL_ROOMS"
        : "TENANT_ROOMS"
      : location.state?.targetTab ||
          (user?.role === "LANDLORD" ? "ALL_ROOMS" : "TENANT_ROOMS"),
  );
  ...
  const fetchAllData = async () => {
    try {
      ...
      if (user?.role === "LANDLORD") {
        ...
        let limit =
          baseLimit === -1 ? -1 : baseLimit + (wallet.extraRoomLimit || 0);
  ...
  const fetchMyReviews = async () => {
    try {
      const res = await contractApi.getMyReviews();
  ...
  const fetchRegulations = async () => {
    try {
      ...
      const filtered = (res.regulations || []).filter(
        (reg) => reg.target === "ALL" || reg.target === user.role,
      );
      setSystemRules(filtered);
    } catch (e) {
      console.error("Lỗi lấy nội quy:", e);
    }
  };
  ...
  const handleEndLease = async (roomId) => {
    ...
    if (!activeContract)
      return alert("Không tìm thấy hợp đồng đang hoạt động!");
  ...
  const handleRequestTermination = async (e) => {
    e.preventDefault();
    try {
      await contractApi.requestTermination(
        terminateData.contractId,
        terminateData,
      );
  ...
  const handleViewRoomDetails = async (room) => {
    ...
    try {
      ...
      if (fullRoom.status === "RENTED") {
        const activeContract = contracts.find(
          (c) => c.roomId == fullRoom.id && c.status === "ACTIVE",
        );
  ...
  const tenantBills = bills.filter(
    (b) => b.tenantIdSnapshot === user?.id || b.contract?.tenantId === user?.id,
  );
  const tenantRoomsMap = new Map();
  tenantBills.forEach((bill) => {
    const actualRoomNumber =
      bill.roomNumberSnapshot ||
  ...
  const totalTenantUnpaid = tenantRoomsList.reduce(
    (sum, r) => sum + r.unpaidCount,
    0,
  );
  ...
  const metrics = useMemo(() => {
    ...
    const upcoming = rooms.filter(
      (r) =>
        ...
        contracts.some(
          (c) =>
            c.roomId === r.id && c.status === "ACTIVE" && c.intendedMoveOutDate,
        ),
    ).length;
  ...
  if (activeTab === "AVAILABLE")
    displayedRooms = rooms.filter(
      (r) =>
        r.status === "AVAILABLE" &&
        !r.isHidden &&
        (!r.depositNote || r.depositNote.trim() === ""),
    );
  if (activeTab === "RENTED")
    displayedRooms = rooms.filter((r) => r.status === "RENTED" && !r.isHidden);
  if (activeTab === "MAINTENANCE")
    displayedRooms = rooms.filter(
      (r) => r.status === "MAINTENANCE" && !r.isHidden,
    );
  ...
  if (activeTab === "DEPOSITED")
    displayedRooms = rooms.filter(
      (r) => r.depositNote && r.depositNote.trim() !== "" && !r.isHidden,
    );
  ...
  if (roomSearchTerm.trim() !== "") {
    ...
    displayedRooms = displayedRooms.filter(
      (r) =>
        (r.roomNumber &&
          String(r.roomNumber).toLowerCase().includes(keyword)) ||
        ...
        if (limit !== -1) {
          ...
          if (visibleCount > limit) {
            setLimitInfo({
              visibleCount,
              limit,
              excessCount: visibleCount - limit,
            });
          } else {
            setLimitInfo(null);
          }
  ...
  if (!user)
    return (
      <div className="flex items-center justify-center h-screen font-black text-primary">
        Đang xác thực...
      </div>
    );
  ...
  return (
    <DashboardProvider
      value={{
        ...
        landlordIncidents,
        transactions,
        fetchLandlordRooms: fetchRooms,
        fetchLandlordContracts: fetchContracts,
        fetchLandlordBills: fetchBills,
      }}
    >
      <div className="flex flex-col h-screen font-['Inter'] bg-surface-container-lowest text-on-surface">
        ...
        <div className="bg-surface-container-lowest text-on-surface px-4 md:px-6 py-3 flex justify-between items-center shadow-sm border-b border-outline-variant/30 z-[100] sticky top-0">
          <div className="flex items-center gap-3">
            ...
            <h2
              className="m-0 cursor-pointer text-primary font-black text-xl md:text-2xl tracking-tight flex items-center gap-1 md:gap-2"
              onClick={() => navigate("/")}
            >
              <span className="material-symbols-outlined text-[24px] md:text-[28px] fill-1">
                real_estate_agent
              </span>
          ...
          <div className="flex items-center gap-4 md:gap-6">
            ...
            <button
              ...
              className="hidden md:flex bg-surface-container-low border border-outline-variant/30 text-on-surface-variant px-4 py-2 rounded-full items-center gap-2 text-[13px] font-black transition-all hover:bg-surface-container-high"
            >
            ...
            <div className="relative" ref={userDropdownRef}>
              ...
              >
                ...
                <span className="text-[14px] max-w-[120px] truncate font-bold hidden md:inline-block">
                  {user?.fullName}
                </span>
                <span className="text-on-surface-variant opacity-60 material-symbols-outlined hidden md:inline-block">
                  expand_more
                </span>
              </div>
              {showDropdown && (
                <div className="absolute top-[120%] right-0 bg-surface-container-lowest border border-outline-variant/30 rounded-2xl p-2 w-[180px] shadow-2xl z-[100] animate-in fade-in slide-in-from-top-2">
                  <div className="px-4 py-3 border-b border-outline-variant/30 mb-1">
                    ...
                    <div className="font-bold text-[10px] text-on-surface-variant uppercase tracking-widest">
                      {user?.role}
                    </div>
                  </div>
                  <div
            ...
            {user?.role === "LANDLORD" && (
              <button
                onClick={() => {
                  if (limitInfo) {
                    alert(
                      "Bạn đã vượt quá giới hạn phòng, vui lòng nâng cấp gói hoặc ẩn phòng!",
                    );
                    return;
                  }
                  setRoomToEdit(null);
        ...
        <div className="flex flex-1 overflow-hidden relative">
          ...
          {/* 2. SIDEBAR */}
          <div
            className={`fixed md:relative top-0 left-0 h-full z-[50] w-[280px] bg-surface-container-lowest border-r border-outline-variant/30 flex flex-col shrink-0 transition-transform duration-300 ${
              showSidebar
                ? "translate-x-0"
          ...
          >
            ...
            <div className="p-4 border-t border-outline-variant/20">
              ...
              >
                ...
                <div>
                  <p className="m-0 text-[10px] font-bold text-on-surface-variant uppercase tracking-widest opacity-60">
                    Hỗ trợ Zalo
                  </p>
                  <p className="m-0 text-xs font-black text-on-surface">
                    Admin - 0337377034
                  </p>
                </div>
              </a>
          ...
          <div className="flex-1 px-4 py-6 md:px-8 md:py-8 overflow-y-auto no-scrollbar relative bg-surface-container-low/30">
            {activeTab === "INCIDENTS" && (
              <IncidentManagement
                ...
                rooms={rooms}
                contracts={contracts}
                onRepairCostUpdated={fetchLandlordIncidents}
              />
            )}

            {activeTab === "WALLET" && user?.role === "LANDLORD" && (
              <WalletTab
                onPurchaseSuccess={checkLimit}
                realtimeRefreshKey={realtimeRefreshKey}
              />
            )}
            ...
            <LandlordBillsTabContent
              ...
              setEditBill={setBillToEdit}
              handleDeleteBill={handleDeleteBill}
            />
            <LandlordRevenueTabContent
              activeTab={activeTab}
              setViewBillDetails={setViewBillDetails}
              setViewIncidentCostDetails={setViewIncidentCostDetails}
            />
        ...
        {selectedImage && (
          <div
            className="fixed inset-0 z-[100000] bg-black/95 flex justify-center items-center"
        ...
        <DepositModal
          ...
          setDepositModal={setDepositModal}
          handleSaveDeposit={handleSaveDeposit}
        ...
        />
        <ViewIncidentCostModal
          viewIncidentCostDetails={viewIncidentCostDetails}
          onClose={() => setViewIncidentCostDetails(null)}
        />
        <ViewContractModal
        ...
        />
        <BillFormModal
          contractForBill={contractForBill}
          onClose={() => setContractForBill(null)}
        />
        <EditBillModal
        ...
        />
        <ContractFormModal
          roomForNewContract={roomForNewContract}
        ...
        />
        <RoomFormModal
          showModal={showRoomFormModal}
          setShowModal={setShowRoomFormModal}
