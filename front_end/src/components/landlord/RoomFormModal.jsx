...
import RoomLocationPickerModal from './RoomLocationPickerModal';
...
const RoomFormModal = ({
  ...
  const [newRoom, setNewRoom] = useState({
    roomNumber: '', houseNumber: '', price: '', description: '', area: '', address: '', latitude: '', longitude: '',
    ...
    if (showModal) {
      if (roomToEdit) {
        setNewRoom({
          ...
          latitude: roomToEdit.latitude ?? '', longitude: roomToEdit.longitude ?? '',
  ...
  const fullAddressPreview = [
    newRoom.houseNumber,
    selectedWard.name || (!selectedWard.code ? newRoom.address?.split(', ')[0] : ''),
    selectedProvince.name || (!selectedProvince.code ? newRoom.address?.split(', ')[1] : '')
  ].filter(Boolean).join(', ');
  const hasCoordinates = newRoom.latitude !== '' && newRoom.latitude !== null && newRoom.latitude !== undefined
    && newRoom.longitude !== '' && newRoom.longitude !== null && newRoom.longitude !== undefined;
  const coordinateText = hasCoordinates ? `${Number(newRoom.latitude).toFixed(6)}, ${Number(newRoom.longitude).toFixed(6)}` : '';
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 md:p-8 bg-on-surface/40 backdrop-blur-md animate-in fade-in duration-300">
          ...
          <fieldset style={{ marginBottom: '20px', borderColor: '#e2e8f0', padding: '15px', borderRadius: '8px' }}>
            ...
            <div className="mt-4 p-4 rounded-2xl bg-surface-container-low border border-outline-variant/30">
              <div className="flex flex-col md:flex-row md:items-end gap-3">
                <div style={{ flex: 1 }}>
                  <label style={{ fontSize: '13px', color: '#64748b', display: 'block', marginBottom: '5px', fontWeight: 600 }}>Tọa độ phòng trên bản đồ</label>
                  <input
                    type="text"
                    readOnly
                    value={coordinateText}
                    placeholder="Chưa chọn vị trí trên bản đồ"
                    style={{ width: '100%', padding: '10px', border: '1px solid #e2e8f0', borderRadius: '6px', boxSizing: 'border-box', background: '#fff', color: coordinateText ? '#0f172a' : '#94a3b8' }}
                  />
                </div>
                <button
                  type="button"
                  onClick={() => setShowLocationPicker(true)}
                  className="px-5 py-3 rounded-xl bg-primary text-on-primary font-black flex items-center justify-center gap-2 hover:scale-[1.02] transition-all"
                >
                ...
                {hasCoordinates && (
                  <button
                    type="button"
                    onClick={() => setNewRoom({ ...newRoom, latitude: '', longitude: '' })}
                    className="px-4 py-3 rounded-xl bg-error/10 text-error font-black hover:bg-error hover:text-white transition-all"
                  >
                    Xóa
                  </button>
                )}
              </div>
              <p className="m-0 mt-2 text-[11px] font-bold text-on-surface-variant opacity-60">
                Tọa độ giúp người thuê xem đúng vị trí phòng trên bản đồ.
              </p>
            </div>
          </fieldset>
      ...
      <RoomLocationPickerModal
        show={showLocationPicker}
        onClose={() => setShowLocationPicker(false)}
        latitude={newRoom.latitude}
        longitude={newRoom.longitude}
        address={fullAddressPreview}
        onConfirm={({ latitude, longitude }) => setNewRoom({ ...newRoom, latitude, longitude })}
      />
    </div>
  );
};
