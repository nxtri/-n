...
const roomController = {
  createRoom: async (req, res) => {
    try {
      ...
      if (!landlord)
        return res.status(404).json({ message: "Không tìm thấy chủ nhà!" });

      // Kiểm tra gói còn hạn không
      const now = new Date();
      if (
        !landlord.subscriptionPlan ||
        landlord.subscriptionPlan === "NONE" ||
        !landlord.subscriptionExpiry ||
        new Date(landlord.subscriptionExpiry) < now
      ) {
        return res
          .status(403)
          .json({
            message:
              "Bạn chưa đăng ký gói dịch vụ hoặc gói đã hết hạn! Vui lòng mua gói để đăng tin.",
          });
      }
      ...
      if (landlord.hasBasePlan) {
        const limitConfig = await SystemConfig.findOne({
          where: { key: `PLAN_${landlord.subscriptionPlan}_LIMIT` },
        });
        if (limitConfig) {
      ...
      let roomLimit =
        baseLimit === -1 ? -1 : baseLimit + (landlord.extraRoomLimit || 0);

      if (roomLimit !== -1 && currentRoomCount >= roomLimit) {
        return res
          .status(403)
          .json({
            message: `Giới hạn gói của bạn chỉ cho phép tối đa ${roomLimit} phòng. Bạn đã có ${currentRoomCount} phòng. Vui lòng nâng cấp gói hoặc mua thêm phòng lẻ!`,
          });
      }
    ...
    try {
      ...
      const processRoom = (r) => {
        ...
        const avgRating =
          reviews.length > 0
            ? (
                reviews.reduce((sum, rev) => sum + rev.rating, 0) /
                reviews.length
              ).toFixed(1)
            : 0;
    ...
    try {
      ...
      if (room.status === "RENTED") {
        return res
          .status(400)
          .json({
            message: "Phòng đang cho thuê, không thể tự ý đổi trạng thái!",
          });
      }
      ...
      res
        .status(200)
        .json({ message: "Cập nhật trạng thái thành công!", room });
    } catch (error) {
      console.error(error);
  ...
  bulkToggleVisibility: async (req, res) => {
    try {
      ...
      await Room.update(
        { isHidden },
        { where: { id: roomIds, landlordId: req.user.id } },
      );
      res
        .status(200)
        .json({ message: "Đã cập nhật trạng thái các phòng thành công!" });
    } catch (error) {
  ...
  updateRoom: async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          message: `Lỗi server khi cập nhật phòng! Chi tiết: ${error.message}`,
        });
    }
  },
  ...
  deleteRoom: async (req, res) => {
    try {
      ...
      if (room.status === "RENTED") {
        return res
          .status(400)
          .json({ message: "Phòng đang cho thuê, không thể xóa!" });
      }
      ...
      res
        ...
        .json({
          message:
            "Lỗi server khi xóa phòng! (Có thể do phòng đang lưu hợp đồng cũ)",
        });
    }
  },
  // =========================================================
  ...
  getPublicRooms: async (req, res) => {
    try {
      ...
      const safeAvailable = availableRooms.filter((r) => {
        ...
        if (
          !r.landlord?.subscriptionPlan ||
          r.landlord.subscriptionPlan === "NONE"
        )
          return false;
        if (
          !r.landlord?.subscriptionExpiry ||
          new Date(r.landlord.subscriptionExpiry) < now
        )
          return false;
        return true;
      });
      ...
      const upcomingRooms = rentedRooms
        .map((room) => {
          if (room.depositNote && room.depositNote.trim() !== "") return null;
          // Kiểm tra gói chủ nhà
          if (
            !room.landlord?.subscriptionPlan ||
            room.landlord.subscriptionPlan === "NONE"
          )
            return null;
          if (
            !room.landlord?.subscriptionExpiry ||
            new Date(room.landlord.subscriptionExpiry) < now
          )
            return null;
          ...
          if (contract && contract.intendedMoveOutDate) {
            return {
              ...room.toJSON(),
              intendedMoveOutDate: contract.intendedMoveOutDate,
            };
          }
          return null;
        })
  ...
  updateDeposit: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Cập nhật ghi chú giữ chỗ thành công!", room });
    } catch (error) {
